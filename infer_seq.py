...
def load_model(model_path, hidden_size=128, num_layers=2):
    ...
    model = AudioSeqModel(input_size=1, hidden_size=hidden_size, num_layers=num_layers)
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.to(device)
    model.eval()
    return model, device

def preprocess_csv(file_path, noise_lambda=0.0):
    """Preprocess a CSV file for inference, optionally adding noise and visualizing."""
    # Read CSV file, skip header rows
    df = pd.read_csv(file_path, skiprows=4)
    time = df.iloc[:, 0].values
    
    # Get probe data as features (use probe1 and scale)
    probe1 = df.iloc[:, 1].values  # Second column is probe 1
    
    # Scale features
    features_scaled = 1e7 * probe1

    # Add Gaussian noise if lambda > 0
    if noise_lambda > 0:
        noise = np.random.normal(0, noise_lambda * np.std(features_scaled), features_scaled.shape)
        features_noisy = features_scaled + noise
    else:
        features_noisy = features_scaled

    # Reshape features to (seq_len, 1)
    features_final = features_noisy.reshape(-1, 1)

    # Visualize the input signal (after scaling and potentially noise)
    plt.figure(figsize=(10, 4))
    plt.plot(time, features_noisy, label=f'Input Signal (Probe 1 * 1e7, Noise Lambda={noise_lambda})')
    plt.xlabel('Time (s)')
    plt.ylabel('Processed Signal Value')
    plt.title('Input Signal for Model')
    plt.grid(True)
    plt.legend()
    plt.savefig('input.png')
    plt.close() # Close the plot to avoid displaying it immediately
    print("Input signal visualization saved to input.png")

    # Convert to tensor (add batch dimension)
    features_tensor = torch.tensor(features_final, dtype=torch.float32).unsqueeze(0)
    seq_length = torch.tensor([features_tensor.shape[1]]) # Get seq_len after reshaping and before unsqueezing
    
    return features_tensor, seq_length
...
def main():
    ...
    parser.add_argument('--noise_lambda', type=float, default=0.0, help='Lambda coefficient for adding Gaussian noise to the input signal')
    ...
    features, seq_length = preprocess_csv(args.csv, args.noise_lambda)
    
    # Make prediction
    print("Making prediction...")
    prediction = predict(model, device, features, seq_length)
    
    # Reverse the scaling applied during training
    f_pred_scaled, z_pred_scaled = prediction
    f_pred = f_pred_scaled * 250
    z_pred = z_pred_scaled * 3000 + 3000
...
def batch_inference():
    ...
    for file_path in csv_files:
        ...
        features, seq_length = preprocess_csv(file_path, args.noise_lambda)
        prediction = predict(model, device, features, seq_length)
        f_pred_scaled, z_pred_scaled = prediction
